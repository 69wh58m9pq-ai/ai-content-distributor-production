...
# PlanetScale Migration Deployment Script
# Run this after PlanetScale database is created and connection string is obtained

set -e

echo "🚀 Starting PlanetScale Migration Deployment"

# Check if DATABASE_URL is provided
if [ -z "$DATABASE_URL" ]; then
    echo "❌ ERROR: DATABASE_URL environment variable is required"
    echo "Please set DATABASE_URL before running this script:"
    echo "export DATABASE_URL='mysql://username:password@aws.connect.psdb.cloud/ai-content-distributor?sslaccept=strict'"
    exit 1
fi
...
echo "✅ Updated .env file with PlanetScale configuration"
...
echo "🚀 Pushing database schema to PlanetScale..."
npx prisma db push
...
git add .
git commit -m "Database: Migrate to PlanetScale MySQL - $(date '+%Y-%m-%d %H:%M:%S')"
