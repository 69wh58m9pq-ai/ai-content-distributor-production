...
# Supabase PostgreSQL Migration Deployment Script
# Run this after Supabase database is created and connection string is obtained

set -e

echo "🚀 Starting Supabase PostgreSQL Migration Deployment"
...
# Update .env file with Supabase PostgreSQL connection
cat > .env << EOF
...
echo "✅ Updated .env file with Supabase PostgreSQL configuration"
...
echo "🚀 Pushing database schema to Supabase PostgreSQL..."
npx prisma db push
...
git add .
git commit -m "Database: Migrate to Supabase PostgreSQL - $(date '+%Y-%m-%d %H:%M:%S')"
